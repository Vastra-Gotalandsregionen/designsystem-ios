...
public struct Recurrence: Codable, Equatable, Hashable, Sendable {
    ...
    public func getRecurringDates(for dateInterval: DateInterval,
        ...
        /// Apply deviations to the recurrence pattern
        for deviation in deviations {

            /// Remove the original entry
            if let index = dates.firstIndex(where: {
                calendar.isDate($0, equalTo: deviation.originalDate, toGranularity: .day)
            }) {
                dates.remove(at: index)
            }

            /// If its adjusted, add the new
            if !deviation.isHidden,
                let adjustedDate = deviation.adjustedDate {
...
public enum RecurrencePeriod: Int, CaseIterable, Identifiable, Codable, Sendable {
    case day = 0
    case week = 1
    case month = 2
...
public enum RecurrenceWeekday: Int, CaseIterable, Identifiable, Codable, Comparable, Sendable {
...
/// `RecurrenceDeviation` represents an adjustment to a recurring date.
/// It is used to handle exceptions in a recurrence pattern where:
/// - A specific occurrence is rescheduled to another date (`adjustedDate`)
/// - A specific occurrence is skipped/hidden (`isHidden`)
///
/// This allows consumers to define deviations without altering the main recurrence rules.
public struct RecurrenceDeviation: Codable, Equatable, Hashable, Sendable {
    /// The original date generated by the recurrence rule.
    public var originalDate: Date

    /// An alternative date to which the original occurrence is moved.
    /// If `nil`, no adjustment is made (unless `isHidden` is true).
    public var adjustedDate: Date? = nil

    /// Indicates if the original occurrence should be hidden/ignored.
    /// Defaults to `false`.
    public var isHidden: Bool = false

    /// Creates a new `RecurrenceDeviation`.
    /// - Parameters:
    ///   - originalDate: The original date in the recurrence.
    ///   - adjustedDate: An optional new date replacing the original date.
    ///   - isHidden: A flag indicating if the occurrence should be skipped.
    public init(original originalDate: Date, adjusted adjustedDate: Date? = nil, isHidden: Bool = false) {
