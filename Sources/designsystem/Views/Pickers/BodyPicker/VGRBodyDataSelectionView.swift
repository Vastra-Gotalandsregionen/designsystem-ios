...
struct VGRBodyDataSelectionView: View {
    ...
    func selectBodyPart(_ part: VGRBodyPart) {
        if let cnt = getContainer(for: part, in: bodyHierarchy) {
            print("identified: ", cnt.id)
            parentBodyPart = cnt
        } else {
            print("not identified (\(part.id))")
        }
    }
...
#Preview("Front") {
    ...
    NavigationStack {
        ScrollView {
            VGRBodyDataSelectionView(orientation: .front, selectedParts: $selectedBodyParts)
        }
        .frame(maxWidth: .infinity)
        .background(.cyan)
